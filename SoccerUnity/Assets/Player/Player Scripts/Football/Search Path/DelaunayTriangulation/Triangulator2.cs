...
//    public class Triangulator2 : IDisposable
//    {
//        #region Primitives
//        /// <summary>
...
//            public NativeArray<int> Ranges { get; set; }
...
//        public Triangulator2(int capacity, Allocator allocator)
//        {
...
//        public Triangulator2(Allocator allocator) : this(capacity: 16 * 1024, allocator) { }
...
//        /// </summary>
//        //public void Run() => new TriangulationJob2(this).Run();
...
//        /// </returns>
//        //public JobHandle Schedule(JobHandle dependencies = default) => new TriangulationJob2(this).Schedule(dependencies);
        
//        public JobHandle Schedule(NativeArray<int> Ranges, NativeArray<float2> Positions)
//        {
//            TriangulationJob2 triangulationJob =  new TriangulationJob2(this);
//            //triangulationJob.input.Positions = Positions;
//            //triangulationJob.input.Ranges = Ranges;
//            return triangulationJob.Schedule(Ranges.Length, 1);
//        }

//        #region Jobs

//        private struct TriangulationJob2 : IJobParallelFor
//        {
...
//            private static readonly ProfilerMarker MarkerValidateInput = new ProfilerMarker("ValidateInput");
//            private static readonly ProfilerMarker MarkerDelaunayTriangulation = new ProfilerMarker("DelaunayTriangulation");
//            private static readonly ProfilerMarker MarkerConstrainEdges = new ProfilerMarker("ConstrainEdges");
//            private static readonly ProfilerMarker MarkerPlantSeeds = new ProfilerMarker("PlantSeeds");
//            private static readonly ProfilerMarker MarkerRefineMesh = new ProfilerMarker("RefineMesh");
//            private static readonly ProfilerMarker MarkerInverseTransformation = new ProfilerMarker("InverseTransformation");

//            public TriangulationJob2(Triangulator2 triangulator)
//            {
...
//                    Ranges = triangulator.Input.Ranges
//                };
...
//                        //Its new
//                        NativeReference<Status>.ReadOnly readOnlyStatusReference = output.Status.AsReadOnly();
//                        var seedPlanter = new SeedPlanter(readOnlyStatusReference, triangles, localPositions, circles, constrainedHalfedges, halfedges);
...
//                using var _hullPrev = hullPrev = new NativeArray<int>(n, Allocator.Temp);
//                using var _hullNext = hullNext = new NativeArray<int>(n, Allocator.Temp);
//                using var _hullTri = hullTri = new NativeArray<int>(n, Allocator.Temp);
//                using var _hullHash = hullHash = new NativeArray<int>(hashSize, Allocator.Temp);

//                using var _ids = ids = new NativeArray<int>(n, Allocator.Temp);
//                using var _dists = dists = new NativeArray<float>(n, Allocator.Temp);

//                using var _EDGE_STACK = EDGE_STACK = new NativeArray<int>(512, Allocator.Temp);
...
//                return !(math.any(e1.xy == e2.xy | e1.xy == e2.yx)) && Triangulator2.EdgeEdgeIntersection(a0, a1, b0, b1);
//            }
...
//                    }
//                    var h2 = NextHalfedge(h1);
//                    if (EdgeEdgeIntersection(edge, new int2(triangles[h1], triangles[h2])))
//                    {
...
//                        break;
//                    }
//                    else if (EdgeEdgeIntersection(edge, new int2(triangles[h1p], triangles[h2p])))
//                    {
//                        unresolvedIntersections.Add(h1p);
//                        tunnelInit = halfedges[h1p];
//                    }
//                    else if (EdgeEdgeIntersection(edge, new int2(triangles[h2p], triangles[h0p])))
//                    {
...
//                }
//                triangles[triangles.Length ^ 3] = pId;
//                triangles[triangles.Length ^ 2] = pathPoints[triangles.Length ^ 1];
//                triangles[triangles.Length ^ 1] = pathPoints[0];
//                circles[triangles.Length ^ 1] = CalculateCircumCircle(pId, pathPoints[pathPoints.Length ^ 1], pathPoints[0], outputPositions.AsArray());
...
//                }
//                var phe = pathHalfedges[pathHalfedges.Length - 1];
...
//                var phe = pathHalfedges[pathHalfedges.Length - 2];
...
//            public SeedPlanter(NativeReference<Status>.ReadOnly status, NativeArray<int> triangles, NativeArray<float2> positions, NativeList<Circle> circles, NativeList<bool> constrainedHalfedges, NativeList<int> halfedges)
//            {
//                this.status = status;
...
//                    {
//                        var (i, j, k) = (triangles[3 * tId + 0], triangles[3 * tId + 1], triangles[3 * tId + 2]);
//                        circles[tId] = CalculateCircumCircle(i, j, k, positions);
//                    }
//                }
//            }
...
//                using var potentialPointsToRemove = new NativeHashSet<int>(capacity: 3 * badTriangles.Length, Allocator.Temp);
...
//                }
//            }
//            void RemoveAt<T>(ref NativeArray<T> array,int length, int index) where T : struct
//            {
//                if (index < 0 || index >= length)
//                {
//                    Debug.LogError("Índice fuera de los límites del array.");
//                    return;
//                }

//                // Desplaza los elementos hacia la izquierda desde el índice a eliminar
//                for (int i = index; i < length - 1; i++)
//                {
//                    array[i] = array[i + 1];
//                }

//                // Decrementa el tamaño efectivo del array
//                length--;
//            }
...
//                {
//                    var tId = badTriangles[t];
//                    RemoveAt(ref triangles, triangles.Length, 3 * tId + 2);
//                    RemoveAt(ref triangles, triangles.Length, 3 * tId + 1);
//                    RemoveAt(ref triangles, triangles.Length, 3 * tId + 0);
//                    //triangles.RemoveAt(3 * tId + 2);
//                    //triangles.RemoveAt(3 * tId + 1);
//                    //triangles.RemoveAt(3 * tId + 0);
//                    circles.RemoveAt(tId);
...
//        private static float Area2(int i, int j, int k, NativeArray<float2> positions)
//        {
...
//        {
//            var (pA, pB, pC) = (positions[i], positions[j], positions[k]);
//            return new Circle(CircumCenter(pA, pB, pC), CircumRadius(pA, pB, pC));
//        }
